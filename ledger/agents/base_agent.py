...
OPENROUTER VERSION — uses openai SDK pointed at OpenRouter.
Model: google/gemini-2.0-flash  |  Env var: OPENROUTER_API_KEY
"""
...
class BaseApexAgent(ABC):
    ...
    """
    def __init__(self, agent_id: str, agent_type: str, store, registry, model="google/gemini-2.0-flash-001"):
    ...
    async def _append_session(self, event: dict):
        ...
        if self.store is not None and self._session_stream is not None:
            try:
                print(f"  [store debug] pool={self.store._pool} stream={self._session_stream}")
                if self.store._pool is None:
                    print("  [store debug] connecting...")
                    await self.store.connect()
                    print(f"  [store debug] connected pool={self.store._pool}")
                ...
                print(f"  [store debug] wrote {event['event_type']} OK")
                return
            except Exception as e:
    ...
    async def _call_llm(self, system: str, user: str, max_tokens: int = 1024):
        """
        Calls Gemini via OpenRouter and returns (text, input_tokens, output_tokens, cost_usd).
        """
        def _sync_call():
            return client_openrouter.chat.completions.create(
                model=self.model,
                messages=[
        ...
        i = resp.usage.prompt_tokens or 0
        o = resp.usage.completion_tokens or 0
        cost = round(i / 1e6 * 0.10 + o / 1e6 * 0.40, 6)
...
class CreditAnalysisAgent(BaseApexAgent):
    ...
    async def _node_analyze(self, state):
        ...
        fin_table = "\n".join([
            f"FY{f['fiscal_year'] if isinstance(f, dict) else ''}: (historical data)" for f in hist
        ]) if hist else "No historical data loaded"
    ...
    async def _node_policy(self, state):
        ...
        if hist:
            ...
            if rev > 0 and d.get("recommended_limit_usd", 0) > rev * 0.35:
                d["recommended_limit_usd"] = int(rev * 0.35)
