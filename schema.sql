-- =========================================================
-- APEX LEDGER - UPDATED SCHEMA
-- =========================================================
-- Purpose:
--   Event store + stream metadata + projection checkpoints
--   + outbox + snapshots + applicant registry seed schema
--
-- Notes:
--   - Explicit indexes are defined in SQL, not only in docs
--   - event_streams includes archive state fields
--   - schema is self-contained and executable
-- =========================================================
...
CREATE TABLE IF NOT EXISTS events (
    ...
    stream_id        TEXT NOT NULL REFERENCES event_streams(stream_id) ON DELETE CASCADE,
...
-- Reviewer-requested explicit indexes
...
-- Helpful composite index for ordered stream replay
...
CREATE TABLE IF NOT EXISTS snapshots (
    ...
    created_at       TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    CONSTRAINT uq_snapshot_stream_position UNIQUE (stream_id, stream_position)
);

CREATE INDEX IF NOT EXISTS idx_snapshots_stream_position
...
CREATE INDEX IF NOT EXISTS idx_financial_history_company_year
...
CREATE INDEX IF NOT EXISTS idx_compliance_flags_active
    ON applicant_registry.compliance_flags (is_active);
...
CREATE INDEX IF NOT EXISTS idx_loan_relationships_year
    ON applicant_registry.loan_relationships (loan_year);